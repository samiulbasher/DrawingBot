...
/* RC-Servo PWM modification: switch between 0.6ms and 2.5ms pulse-width at 61Hz
   ...
   Make sure that the line below exists in cpu_map.h (introduce 61Hz 1/1024 prescaler)
...
#ifdef SERVO_SUPPORT
  #define SERVO_SHORT   15   // set min pulse duration to (7 = 0.5ms, 15 = 1.03ms, 20=1.40ms)
  #define SERVO_LONG    32   // set max pulse duration (38 = 2.49ms, 31 = 2.05ms)
#endif
...
void spindle_init()
{
    ...
    SPINDLE_TCCRB_REGISTER = SPINDLE_TCCRB_INIT_MASK; // For servo
...
void spindle_stop()
{
  ...
  void spindle_set_speed(uint8_t pwm_value)
  {
    ...
    uint8_t spindle_compute_pwm_value(float rpm) // 328p PWM register is 8-bit.
    {
      ...
      #ifdef SERVO_SUPPORT    
        /* 
         * Recalculate the OCR value for servo control
        */                                 
        #define SERVO_RANGE         (SERVO_LONG - SERVO_SHORT)  
        #define SPINDLE_RPM_RANGE   (DEFAULT_SPINDLE_RPM_MAX - DEFAULT_SPINDLE_RPM_MIN)                                                 
        
        #ifdef SERVO_INVERT                                                                            
          pwm_value = floor(SERVO_LONG - rpm * (SERVO_RANGE / SPINDLE_RPM_RANGE));    
        #else                                                                                             
          pwm_value = floor(rpm * (SERVO_RANGE / SPINDLE_RPM_RANGE) + SERVO_SHORT);   
        #endif  
      #endif                                                                                           
